...
## Upgrade Path: 7.4.5 → 7.5.0 (RHEL 9.x OS Upgrade) → 7.6.5
...
-   7.4.5 → 7.5.0 uses the **in-place upgrade script (.bin method)**.
-   7.5.0 and later uses the **Upgrade Framework (csadm upgrade)**.
-   7.5.0 includes **OS migration to RHEL 9.x**.
...
## 1. Download In-Place Upgrade Script
...
This step includes OS migration to RHEL 9.x.
...
Expected: - FortiSOAR 7.5.0 - RHEL 9.x

Logout and login again in GUI.

------------------------------------------------------------------------

# PHASE 3 --- Upgrade 7.5.0 → 7.6.5 (Upgrade Framework)
...
Log file:
/var/log/cyops/upgrade-fortisoar-7.6.5-`<timestamp>`{=html}.log
